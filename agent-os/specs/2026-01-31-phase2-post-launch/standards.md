...
## Gmail API
- Rate limiter: 250 units/second token bucket
- Retry: exponential backoff, max 3 attempts
- messages.modify for label changes (archive, mark read)
- messages.trash for delete (recoverable 30 days)

## Security
- Tokens stored in macOS Keychain via flutter_secure_storage
- Keychain access requires code signing with Apple Developer certificate
- App sandbox enabled with network.client and network.server entitlements

## Categorization
- 8 categories: personal, social, newsletter, notification, marketing, transactional, automated, unknown
- 5-layer heuristic: Gmail labels > sender domains > sender patterns > subject keywords > unsubscribe header
- Category stored as text string in sender_table (matches enum name)
